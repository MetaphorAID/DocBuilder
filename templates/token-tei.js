(function () {
		...
		each('s,l', function (s, si) {
			...
			each('token', function (w, wi) {
				...
				if (tv) {
					...
					ew.appendChild(et);
					let ana = sel('ana[correct="True"]', w);
	...
	function updJoin(tid, xtl) {
		let l = tid > 0 && ['right', 'both'].indexOf(xtl[tid - 1].getAttribute('join')) !== -1;
		let r = tid < xtl.length - 1 && ['left', 'both'].indexOf(xtl[tid + 1].getAttribute('join')) !== -1;
	...
	function updAna(tokens, cid) {
		...
		fetch('https://emtsv.elte-dh.hu/morph', {
			...
			let tpl = sel('ana', _active[cid]);
			for (let d in data) {
				d = data[d];
				let ana = parseXml(tpl.outerHTML).documentElement;
	...
	document.addEventListener('click', function (e) {
		...
		if (t.matches('.t, .cfg')) {
			...
			if (xt) { //token
				...
				if (_annots.xml) {
					...
					let has = false;
					let lst = {};
					let items = _annots.ref[xt.getAttribute('xml:id')];
					for (let i in items || {}) {
				...
				let tkn = selToText(xt, 'form', true);
				if (tkn.length > 1) {
					let split = {};
		...
		if (t.matches('.edit.ana')) {
			...
			let ana = sel('ana[modified="True"]', xt);
			html += '<tr>'
		...
		if (t.matches('.save.ana')) {
			let morph = sel('morph', xt);
			let mod = sel('ana[modified="True"]', morph);
			if (mod) delNode(mod);
			if (!sel('ana[correct="True"]', morph)) {
				let input = sel('.selAna.selected', t.closest('.tooltip'));
				if (!input) {
		...
		if (t.matches('.ins2.token')) {
			...
			let tkn = sel('form', xt2);
			tkn.textContent = val;
			tkn.setAttribute('modified', 'True');
			let morph = sel('morph', xt2);
			if (morph) {
				morph.setAttribute('check', 'False');
				morph.innerHTML = '';
			}
	...
	document.addEventListener('change', function (e) {
		...
		if (t.matches('.split.token')) {
			...
			let xt2 = parseXml(xt.outerHTML).documentElement;
			sel('form', xt2).textContent = tkn.innerHTML.slice(val);
		...
		// 	let xt2 = x.createElement(val);
		// 	xt2.innerHTML = xt.innerHTML;
		// 	sel('form', xt2).setAttribute('modified', 'True');
		// 	let id = x.documentElement.getAttribute('xml:id');
		// 	if (id) {
		...
		if (t.matches('.add.annot')) {
			...
			let i = _annots.xml.createElement(xt.nodeName);
			i.setAttribute('target', xt.getAttribute('xml:id'));
			i.textContent = sel('form', xt).textContent;
			let a = _annots.xml.createElement('annotation');
		...
		if (t.matches('.addto.annot')) {
			if (val === '' || !_annots.xml) return;
			let an = _annots.list[val.slice(1)];
			let i = _annots.xml.createElement(xt.nodeName);
			i.setAttribute('target', xt.getAttribute('xml:id'));
			i.textContent = sel('form', xt).textContent;
