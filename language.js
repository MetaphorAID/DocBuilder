class LanguageManager extends EventTarget {
	...
	translate(text) {
		...
		return Object.hasOwn(locale, text) ? locale[text] : text;
	}
	...
	#assertSupported(language) {
		if (!this.#isSupported(language)) {
			throw new RangeError(`Unsupported language: ${String(language)}`);
		}
	}
	...
	#validateLanguages(languages) {
		...
		for (const [code, definition] of Object.entries(languages)) {
			if (!definition || typeof definition !== 'object') {
				throw new TypeError(`Invalid language definition: ${code}`);
			}
			...
			for (const [key, translation] of Object.entries(definition.locale)) {
				if (typeof translation !== 'string') {
					throw new TypeError(`Translation must be a string: ${code}.${key}`);
				}
			}
...
const languageManager = new LanguageManager({
	en: {label: 'ENG', locale: {}},
	hu: {label: 'HU', locale: window.Locale}
});
