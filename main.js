...
class UndoManager {
	...
	async #applyHistoryEntry(entry, from, to, editorState) {
		...
		const {chunksToSave, hiddenToRender} = this.#applyChunksToEditor(entry);
		let renderCids;
		let renderHidden;
	...
	#apply(reverse, editorState) {
		...
		if (entry.id !== this.#editor.id) {
			const err = new Error(_('Undo/redo history belongs to another document; history was cleared.'));
